...
默认开启，会在 `DE-like` 与 `TCN` 之间加入轻量空间图卷积残差分支。

### 开关与参数

1. `--use_gcn`：显式启用 Spatial GCN（默认已启用）。
2. `--no_gcn`：关闭 Spatial GCN（用于基线对照）。
3. `--gcn_hidden`：GCN 隐层维度，默认 `16`。
4. `--gcn_beta`：`I` 与学习邻接融合系数，默认 `0.2`，范围 `[0,1]`。
5. `--gcn_dropout`：GCN 分支 dropout，默认 `0.1`。
